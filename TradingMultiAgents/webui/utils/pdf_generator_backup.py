...
class PDFReportGenerator:
    ...
    def _create_debate_transcript_section(self, transcript: str) -> list:
        ...
        title = "🗣️ Bull vs Bear 議論記録" if self.japanese_font in ['HeiseiKakuGo-W5', 'HeiseiMin-W3', 'JapaneseFont'] else "Bull vs Bear Debate Transcript"
        story.append(Paragraph(title, self.styles['CustomSubtitle']))
        story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#e5e7eb')))
        story.append(Spacer(1, 0.2*inch))
        ...
        for line in lines:
            if line.strip():
                ...
                if '🐂' in line or 'Bull' in line:
                    ...
                    story.append(Paragraph(safe_line, speaker_style))
                elif '🐻' in line or 'Bear' in line:
                    speaker_style = ParagraphStyle(
                        name='BearSpeaker',
                        parent=self.styles['SectionHeader'],
    ...
    def _create_risk_discussion_section(self, discussion: str) -> list:
        ...
        title = "⚖️ リスク管理チーム議論" if self.japanese_font in ['HeiseiKakuGo-W5', 'HeiseiMin-W3', 'JapaneseFont'] else "Risk Management Team Discussion"
        story.append(Paragraph(title, self.styles['CustomSubtitle']))
        story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#e5e7eb')))
        story.append(Spacer(1, 0.2*inch))
        ...
        for line in lines:
            if line.strip():
                ...
                # リスクアナリストタイプを識別
                if '🚀' in line or '積極派' in line or 'Aggressive' in line:
                    ...
                    story.append(Paragraph(safe_line, analyst_style))
                elif '🛡️' in line or '保守派' in line or 'Conservative' in line:
                    analyst_style = ParagraphStyle(
                    ...
                    story.append(Paragraph(safe_line, analyst_style))
                elif '⚖️' in line or '中立派' in line or 'Neutral' in line:
                    analyst_style = ParagraphStyle(
                        name='NeutralAnalyst',
                        parent=self.styles['SectionHeader'],
    ...
    def _create_key_metrics_section(self, metrics: str) -> list:
        ...
        title = "📊 主要指標サマリー" if self.japanese_font in ['HeiseiKakuGo-W5', 'HeiseiMin-W3', 'JapaneseFont'] else "Key Metrics Summary"
        story.append(Paragraph(title, self.styles['CustomSubtitle']))
        story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#e5e7eb')))
        story.append(Spacer(1, 0.2*inch))
            ...
            if section.strip():
                ...
                if lines:
                    ...
                    if lines[0].startswith('#'):
                        ...
                        safe_header = self._make_text_safe(header)
                        story.append(Paragraph(safe_header, self.styles['SectionHeader']))
                        story.append(Spacer(1, 0.1*inch))
                        ...
                        for line in lines[1:]:
                            if line.strip() and line.startswith('-'):
                                ...
                                if ':' in metric_line:
                                    ...
                                    table_data.append([
                                        self._make_text_safe(parts[0].strip()),
                                        self._make_text_safe(parts[1].strip())
                                    ])
                        
                        if table_data:
                            # テーブル作成
                            metrics_table = Table(table_data, colWidths=[3*inch, 3*inch])
    ...
    def _create_action_items_section(self, action_items: str) -> list:
        ...
        title = "✅ 推奨アクション項目" if self.japanese_font in ['HeiseiKakuGo-W5', 'HeiseiMin-W3', 'JapaneseFont'] else "Recommended Action Items"
        story.append(Paragraph(title, self.styles['CustomSubtitle']))
        story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#e5e7eb')))
        story.append(Spacer(1, 0.2*inch))
        ...
        for section in sections:
            if section.strip():
                ...
                if lines and lines[0].startswith('#'):
                    ...
                    # 優先度に基づいて色を設定
                    if '優先' in header or 'Priority' in header or '🚨' in header:
                    ...
                    for line in lines[1:]:
                        if line.strip():
                            ...
                            if '🔴' in line:
                                ...
                                item_style = ParagraphStyle(
                                    name='MediumPriorityItem',
                                    parent=self.styles['CustomBody'],
                                    textColor=colors.HexColor('#f59e0b'),
                                    fontName=self.japanese_font
                                )
                            else:
                                item_style = self.styles['CustomBody']
                            
                            story.append(Paragraph(safe_line, item_style))
                            story.append(Spacer(1, 0.05*inch))
