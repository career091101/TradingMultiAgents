...
class MessageBuffer:
    ...
    def _update_current_report(self):
        ...
        if latest_section and latest_content:
            ...
            section_titles = {
                ...
                "news_report": "ニュース分析",
                "fundamentals_report": "ファンダメンタル分析",
                "investment_plan": "リサーチチーム決定",
                "trader_investment_plan": "トレーディングチーム計画",
                "final_trade_decision": "ポートフォリオ管理決定",
            }
            self.current_report = (
    ...
    def _update_final_report(self):
        ...
        if any(
            ...
            report_parts.append("## アナリストチームレポート")
        ...
        if self.report_sections["investment_plan"]:
            report_parts.append("## リサーチチーム決定")
            report_parts.append(f"{self.report_sections['investment_plan']}")

        # Trading Team Reports
        if self.report_sections["trader_investment_plan"]:
            report_parts.append("## トレーディングチーム計画")
            report_parts.append(f"{self.report_sections['trader_investment_plan']}")

        # Portfolio Management Decision
        if self.report_sections["final_trade_decision"]:
            report_parts.append("## ポートフォリオ管理決定")
...
def update_display(layout, spinner_text=None):
    ...
    layout["header"].update(
        Panel(
            "[bold green]TradingAgents CLIへようこそ[/bold green]\n"
            "[dim]© [Tauric Research](https://github.com/TauricResearch)[/dim]",
            title="TradingAgentsへようこそ",
            border_style="green",
            padding=(1, 2),
            expand=True,
        )
...
def get_user_selections():
    ...
    def create_question_box(title, prompt, default=None):
        ...
        if default:
            box_content += f"\n[dim]デフォルト: {default}[/dim]"
        return Panel(box_content, border_style="blue", padding=(1, 2))

    # Step 1: Ticker symbol
    console.print(
        create_question_box(
            "ステップ 1: ティッカーシンボル", "分析するティッカーシンボルを入力してください", "SPY"
        )
    )
    ...
    console.print(
        create_question_box(
            "ステップ 3: アナリストチーム", "分析に使用するLLMアナリストエージェントを選択してください"
        )
    )
    selected_analysts = select_analysts()
    console.print(
        f"[green]選択されたアナリスト:[/green] {', '.join(analyst.value for analyst in selected_analysts)}"
    )

    # Step 4: Research depth
    console.print(
        create_question_box(
            "ステップ 4: 研究深度", "研究深度レベルを選択してください"
        )
    )
    ...
    console.print(
        create_question_box(
            "ステップ 5: OpenAIバックエンド", "使用するサービスを選択してください"
        )
    )
    ...
    console.print(
        create_question_box(
            "ステップ 6: 思考エージェント", "分析に使用する思考エージェントを選択してください"
        )
    )
    ...
    while True:
        ...
        try:
            ...
            if analysis_date.date() > datetime.datetime.now().date():
                console.print("[red]エラー: 分析日は未来の日付にできません[/red]")
                continue
            return date_str
        except ValueError:
            console.print(
                "[red]エラー: 無効な日付形式です。YYYY-MM-DD形式で入力してください[/red]"
            )


def display_complete_report(final_state):
    """Display the complete analysis report with team-based panels."""
    console.print("\n[bold green]完全分析レポート[/bold green]\n")
